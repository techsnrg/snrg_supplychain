	...
	add_action_buttons: function(frm) {
		...
		frm.add_custom_button(__('Calculate Freight'), function() {
			...
			let do_calculate = function() {
				frappe.call({
					method: 'snrg_supplychain.supply_chain.doctype.freight_quotation.freight_quotation.calculate_freight',
		...
		if (has_selection && frm.doc.freight_options.length > 0) {
			frm.add_custom_button(__('Finalize Selection'), function() {
				frappe.confirm(
					...
					function() {
						frappe.call({
							method: 'snrg_supplychain.supply_chain.doctype.freight_quotation.freight_quotation.finalize_selection',
							args: { name: frm.doc.name },
							freeze: true,
							callback(r) {
								if (!r.exc && r.message) {
